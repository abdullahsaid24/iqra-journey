...
import { LessonPassFailChart } from "@/components/stats/LessonPassFailChart";
import { CurrentLessonsTable } from "@/components/stats/CurrentLessonsTable";
import CountdownTimer from "@/components/stats/CountdownTimer";
import { useUserRole } from "@/hooks/useUserRole";
import { Carousel, CarouselContent, CarouselItem, CarouselNext, CarouselPrevious } from "@/components/ui/carousel";
import { Card, CardContent } from "@/components/ui/card";
import { format } from "date-fns";
import { useState } from "react";
import { GoalProgressBar } from "@/components/stats/GoalProgressBar";
...
const StudentStats = () => {
  ...
  const handleBack = () => {
    ...
    if (isParentView) {
      ...
      if (classMatch) {
        navigate(`/class/${studentData?.class_id || ''}`);
      } else {
        navigate(-1);
      }
    }
